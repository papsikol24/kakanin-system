...
// Enhanced orders query with tracking information
$orders = $pdo->prepare("
    SELECT o.*, 
           CASE 
...
$activeOrders = array_filter($orders, function($o) {
    return in_array($o['status'], ['pending', 'confirmed', 'preparing', 'ready', 'out_for_delivery']);
});
...
// Pass order data to JavaScript for real-time tracking
$orderData = [];
foreach ($orders as $order) {
    $orderData[$order['id']] = [
        'status' => $order['status'],
        'is_deleted' => ($order['customer_id'] == null && !empty($order['customer_name'])) ? 1 : 0,
...
<style>
    ...
    * {
    ...
    .store-status-banner.status-updated {
    ...
    .btn-order-now-mobile:hover:not(.disabled) {
        transform: translateY(-2px);
        box-shadow: 0 8px 20px rgba(255, 140, 0, 0.3);
        color: white;
    }

    .btn-order-now-mobile.disabled {
        opacity: 0.5;
        cursor: not-allowed;
        pointer-events: none;
        background: #6c757d;
...
<div class="container mt-2">
    ...
    <div class="alert alert-warning mb-3" id="storeClosedWarning">
        ...
        <br><small>You can still browse products, but ordering is disabled until the store opens.</small>
    </div>
    <?php endif; ?>
    ...
    <div class="welcome-card">
        <div class="welcome-row">
            ...
            <div class="welcome-actions">
                ...
                <!-- ORDER NOW BUTTON -->
                <a href="/menu" class="btn-order-now-mobile <?php echo !$store_online ? 'disabled' : ''; ?>" id="orderNowBtn">
                ...
                <?php foreach ($notifications as $notif): ?>
                    <div class="notification-item <?php echo $notif['is_read'] ? '' : 'unread'; ?>" data-notification-id="<?php echo $notif['id']; ?>">
                        ...
                        <?php if ($notif['order_id']): ?>
                            ...
                            $notif_order = null;
                            foreach ($orders as $o) {
                                if ($o['id'] == $notif['order_id']) {
                                    $notif_order = $o;
                                    break;
                                }
                            }
                            ?>
                            <?php if ($notif_order && !in_array($notif_order['status'], ['delivered', 'completed', 'cancelled'])): ?>
                                <a href="/order-tracking.php?id=<?php echo $notif['order_id']; ?>" class="btn-track-action" style="margin-top: 0.5rem; display: inline-block; font-size: 0.7rem; padding: 0.2rem 0.5rem;">Track</a>
                            <?php else: ?>
    ...
    <div class="best-sellers-grid" id="bestSellersGrid">
        ...
        <div class="best-seller-card" data-product-id="<?php echo $productId; ?>" data-product-name="<?php echo htmlspecialchars($productName); ?>" data-product-price="<?php echo $productPrice; ?>">
            ...
            if (!empty($productImage) && file_exists($_SERVER['DOCUMENT_ROOT'] . $imagePath)): 
            ?>
            ...
            <div class="best-seller-info">
                ...
                <?php if ($current_stock > 0): ?>
                    <?php if (!$store_online): ?>
                        <button class="btn-add-cart-small disabled" disabled>
                            <i class="fas fa-store-slash"></i> Store Closed
                        </button>
                    <?php else: ?>
                        <button class="btn-add-cart-small add-to-cart-btn store-aware" 
    ...
    <div class="order-history-title">
        <i class="fas fa-history"></i> Your Orders
    ...
    <?php if (empty($orders)): ?>
        <div class="alert alert-info">
            ...
            No orders yet. 
            <a href="/menu" class="alert-link">Browse menu</a>
        </div>
    <?php else: ?>
        <div class="table-container">
            <div class="table-header" id="ordersTableHeader">
                <div>
                ...
                <table class="table" id="ordersTable">
                    ...
                    <tbody>
                        <?php foreach ($orders as $order): 
                            ...
                            $minutesAgo = round((time() - $orderTime) / 60);
                            $isNew = ($order['status'] == 'pending' && $minutesAgo <= 30);
                            ...
                            if ($order['tracking_number']) {
                                $trackingDisplay = '<span class="tracking-number" title="Tracking #">' . $order['tracking_number'] . '</span>';
                            }
                            
                            // Define final states
                            $final_states = ['delivered', 'completed', 'cancelled'];
                        ?>
                        <tr data-order-id="<?php echo $order['id']; ?>" data-status="<?php echo $order['status']; ?>" class="<?php echo $isNew ? 'new-order' : ''; ?>">
                            <td>
                            ...
                            <td>
                                <?php echo $trackingDisplay ?: '<span class="text-muted">—</span>'; ?>
                            </td>
                            <td>
                                <?php if (!in_array($order['status'], $final_states)): ?>
                                    <a href="/order-tracking.php?id=<?php echo $order['id']; ?>" class="btn-action btn-track-action" title="Track Order Live">
                                        <i class="fas fa-map-marker-alt"></i> Track
                                    </a>
                                <?php else: ?>
                                    <a href="/order-success.php?id=<?php echo $order['id']; ?>" class="btn-action btn-view-action" title="View Details">
...
<script>
// ===== INITIAL ORDER DATA FOR REALTIME TRACKING =====
let initialOrderData = <?php echo $orderDataJson; ?>;
let lastOrderData = {...initialOrderData};
...
function updateStoreUI(data) {
    ...
    const addButtons = document.querySelectorAll('.store-aware');
    
    if (!banner) return;
    
    // Check if status actually changed
    const wasOnline = banner.classList.contains('online');
    ...
    if (wasOnline !== isOnline) {
        ...
        setTimeout(() => banner.classList.remove('status-updated'), 1000);
    }
    ...
    // Update all store-aware buttons (add to cart buttons)
    addButtons.forEach(btn => {
        if (!isOnline) {
            ...
            const inCart = btn.closest('.best-seller-card')?.querySelector('.cart-indicator-small');
            btn.innerHTML = '<i class="fas fa-cart-plus"></i> ' + (inCart ? 'Add More' : 'Add to Cart');
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.add-to-cart-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            const isStoreOnline = !document.getElementById('orderNowBtn')?.classList.contains('disabled');
            if (!isStoreOnline) {
...
// ===== REAL-TIME ORDER STATUS UPDATE FUNCTION =====
function updateOrdersTable(orders) {
    ...
    const finalStates = ['delivered', 'completed', 'cancelled'];
    ...
    orders.forEach(order => {
        ...
        if (!previousData || previousData.status !== currentStatus) {
            ordersChanged = true;
            console.log(`📝 Order #${orderId} status changed from ${previousData?.status || 'new'} to ${currentStatus}`);
        }
    });
    ...
    orders.forEach(order => {
        ...
        const trackingDisplay = order.tracking_number ? 
            '<span class="tracking-number" title="Tracking #">' + order.tracking_number + '</span>' : 
        ...
        html += `<tr data-order-id="${order.id}" data-status="${order.status}" class="${isNew ? 'new-order' : ''}">
            ...
            <td>
                ${!finalStates.includes(order.status) ?
                    `<a href="/order-tracking.php?id=${order.id}" class="btn-action btn-track-action" title="Track Order">
                        <i class="fas fa-map-marker-alt"></i> Track
                    </a>` :
    ...
    // Flash the table header
    const tableHeader = document.getElementById('ordersTableHeader');
    if (tableHeader) {
...
function updateNotificationsPanel(notifications) {
    ...
    notifications.forEach(n => {
        ...
        html += `<div class="notification-item ${isUnread ? 'unread' : ''}">
            ...
            ${n.order_id ? 
                (!finalStates.includes(n.order_status) ?
                    `<a href="/order-tracking.php?id=${n.order_id}" class="btn-track-action" style="margin-top:0.5rem; display:inline-block; font-size:0.7rem; padding:0.2rem 0.5rem;">Track</a>` :
                    `<a href="/order-success.php?id=${n.order_id}" class="btn-view-action" style="margin-top:0.5rem; display:inline-block; font-size:0.7rem; padding:0.2rem 0.5rem;">View</a>`
                ) : ''}
        </div>`;
    });
...
function fetchCustomerUpdates() {
    ...
    fetch('/api/get_realtime_data.php?device_id=' + encodeURIComponent(deviceId) + '&t=' + Date.now())
        .then(res => res.json())
        .then(data => {
            console.log('Customer real-time data:', data);
            
            if (!data.success || data.userType !== 'customer') {
                isUpdating = false;
                return;
            }
            ...
            if (data.unreadCount !== undefined) {
                ...
                // Check if count increased (for sound trigger)
                if (data.unreadCount > lastNotificationCount) {
...
updateInterval = setInterval(fetchCustomerUpdates, 3000);
